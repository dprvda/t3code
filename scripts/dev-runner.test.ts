...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("createDevRunnerEnv", () => {
      ...
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            ...
            VITE_WS_URL: "ws://localhost:14773",
          },
          serverOffset: 0,
          webOffset: 0,
          t3Home: "/tmp/my-t3",
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3CODE_PORT, "14773");
        assert.equal(env.PORT, "6733");
      }),
    );
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VITE_HTTP_URL, "http://127.0.0.1:14773");
      }),
    );
    ...
    it.effect("clears the single-origin marker in dev:server mode", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3CODE_SINGLE_ORIGIN_DEV, undefined);
        assert.equal(env.VITE_HTTP_URL, "http://localhost:14773");
      }),
    );
    ...
    it.effect("keeps explicit backend URLs for the desktop renderer", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.VITE_HTTP_URL, "http://127.0.0.1:14773");
        assert.equal(env.VITE_WS_URL, "ws://127.0.0.1:14773");
      }),
    );
  });
  ...
  describe("findFirstAvailableOffset", () => {
    ...
    it.effect("advances until all required ports are available", () =>
      Effect.gen(function* () {
        const taken = new Set([14773, 6733, 14774, 6734]);
        const offset = yield* findFirstAvailableOffset({
          startOffset: 0,
          requireServerPort: true,
          requireWebPort: true,
    ...
    it.effect("skips browser-blocked web ports before probing availability", () =>
      Effect.gen(function* () {
        ...
        const offset = yield* findFirstAvailableOffset({
          // 6733 + 833 = 6566, which browsers block as sane-port.
          startOffset: 833,
          requireServerPort: true,
          requireWebPort: true,
  ...
  describe("isBrowserAllowedPort", () => {
    ...
    it.each([6733, 5900, 7566, 6670, 8733])("allows browser-safe web port %s", (port) => {
  ...
  describe("resolveModePortOffsets", () => {
    it.effect("uses a shared fallback offset for dev mode", () =>
      Effect.gen(function* () {
        const taken = new Set([14773, 6733]);
  ...
  describe("runDevRunnerWithInput", () => {
    ...
    it.effect("ignores an ambient T3CODE_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { T3CODE_HOME: "/home/user/.carbon-studio" },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
          browser: undefined,
    ...
    describe("t3 home precedence", () => {
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            ...
            cwd: root,
            ambientHome: "/home/user/.carbon-studio",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".carbon-studio"));
        }).pipe(Effect.scoped),
      );
      ...
      it.effect("falls back to an ambient T3CODE_HOME outside a worktree", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.resolve("/home/user/.carbon-studio"));
        }),
      );
