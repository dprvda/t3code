...
import { LaunchDocsDialog } from "./LaunchDocsDialog";
...
export function DocBricksPicker(props: DocBricksPickerProps) {
  ...
  const [isLaunchDocsOpen, setIsLaunchDocsOpen] = useState(false);
    ...
    <>
      ...
      >
        ...
        <PopoverPopup align="start" className="w-80 p-2">
          ...
          <div className="mt-1 border-border border-t px-1.5 pt-1.5">
            <button
              type="button"
              data-chat-launch-docs-open="true"
              className="text-secondary-label text-xs underline-offset-2 hover:text-foreground hover:underline"
              onClick={() => {
                setIsOpen(false);
                setIsLaunchDocsOpen(true);
              }}
            >
              Session-start docs…
            </button>
      ...
      <LaunchDocsDialog
        environmentId={props.environmentId}
        cwd={props.cwd}
        open={isLaunchDocsOpen}
        onOpenChange={setIsLaunchDocsOpen}
      />
    </>
  );
}
