...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    unitPath: "/home/theo/.config/systemd/user/carbon-studio.service",
  });
...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.carbonstudio.app.service.plist",
};
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now carbon-studio.service")).toEqual(
  ...
  it.effect("restarts an installed service when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("systemctl "))).toEqual([
        "systemctl --user stop carbon-studio.service",
        "systemctl --user daemon-reload",
        "systemctl --user restart carbon-studio.service",
      ]);
    }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith("Library/LaunchAgents/com.carbonstudio.app.service.plist"),
      ).toBe(true);
      expect(parseServiceState(yield* fs.readFileString(statePath))).toEqual({
        protocol: SERVICE_LAUNCHER_PROTOCOL,
        activeVersion: "1.2.3",
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.carbonstudio.app.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.carbonstudio.app.service",
        "launchctl enable gui/501/com.carbonstudio.app.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install;
      control.failCommand = "launchctl bootout --wait gui/501/com.carbonstudio.app.service";
