...
export const WORKTREE_BRANCH_PREFIX = "carbon-studio";
const LEGACY_WORKTREE_BRANCH_PREFIX = "t3code";
// Canonical form is `carbon-studio/<8 hex>`. Older mobile builds generated
// `<prefix>/<uuid>` via Crypto.randomUUID() (always RFC 4122 v4), so the matcher
// accepts that shape too. Threads created before the product rename still carry
// `t3code/` placeholders; accept that legacy prefix while all new names use the
// Carbon Studio prefix.
